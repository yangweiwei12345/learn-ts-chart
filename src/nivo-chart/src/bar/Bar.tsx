...
export const Bar = <RawDatum extends BarDatum>({
    isInteractive = svgDefaultProps.isInteractive,    // 是否启用交互性 default true，(Tooltip 提示)
    animate = svgDefaultProps.animate,                // 动画，只支持svg，不支持canvas，api
    motionConfig = svgDefaultProps.motionConfig,      // react-spring的运动配置，可以是preset（默认配置），也可以是自定义配置custom。
    theme,
    renderWrapper,
    ...otherProps
}: BarSvgProps<RawDatum>) => (
    <Container    // 配置context，主题，动画
        {...{
            animate,
            isInteractive,
            motionConfig,
