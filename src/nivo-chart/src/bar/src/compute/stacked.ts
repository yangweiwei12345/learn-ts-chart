...
const generateVerticalStackedBars = <RawDatum extends Record<string, unknown>>(
    ...
    stackedData.forEach(stackedDataItem =>    // stackedDataItem => [[0, 58], [0, 141], ...]
        /* xScale.domain() => ['AD', 'AE', 'AF', 'AG', 'AI', 'AL', 'AM'] 
            index => 'AD'
         */
        xScale.domain().forEach((index, i) => {
            console.log(JSON.stringify(stackedDataItem), 'stackedDataItem')
            const d = stackedDataItem[i]    // d => [0, 58]
...
> & {
    ...
    console.log(indexScale, 'indexScale')
    ...
    const bandwidth = indexScale.bandwidth()    // 每个分段的宽度
